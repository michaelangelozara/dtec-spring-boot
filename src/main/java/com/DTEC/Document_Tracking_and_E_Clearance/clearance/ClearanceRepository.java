...
public interface ClearanceRepository extends JpaRepository<Clearance, Integer> {
    ...
    @Query("SELECT c FROM Clearance c JOIN c.user u WHERE (u.firstName LIKE %:search% OR u.lastname LIKE %:search%) AND c.status != 'COMPLETED'")
    List<Clearance> findAllBySearch(@Param("search") String search);
...
//    @Query("SELECT c FROM Clearance c WHERE c.type = 'STUDENT_CLEARANCE' AND c.status = 'COMPLETED' AND c.isClearancePermitReleased = false")
    @Query("SELECT c FROM Clearance c WHERE c.type = 'STUDENT_CLEARANCE' AND c.status = 'COMPLETED'")
