...
public interface PermitToEnterRepository extends JpaRepository<PermitToEnter, Integer> {
    ...
    @Query("SELECT p FROM PermitToEnter p LEFT JOIN p.club c LEFT JOIN c.memberRoles mr WHERE mr.id =:userId AND STR(mr.role) =:role")
    Page<PermitToEnter> findAll(Pageable pageable, @Param("userId") int userId, @Param("role") String role);

    @Query("SELECT p FROM PermitToEnter p LEFT JOIN p.signedPeople sp WHERE STR(p.currentLocation) =:currentLocation AND sp.user.id =:userId")
