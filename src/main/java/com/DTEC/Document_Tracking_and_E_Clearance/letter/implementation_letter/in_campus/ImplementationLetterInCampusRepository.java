...
public interface ImplementationLetterInCampusRepository extends JpaRepository<ImplementationLetterInCampus, Integer> {
    ...
    @Query("SELECT i FROM ImplementationLetterInCampus i LEFT JOIN i.signedPeople sp LEFT JOIN sp.user u WHERE (u.firstName LIKE %:query% OR u.lastname LIKE %:query%) AND u.id =:id")
    List<ImplementationLetterInCampus> findAllOIC(@Param("query") String query, @Param("id") int id);
}
